...
#    pikes_entities_data_captions=[]
    for j in range(5):        
        ...
        for row in entities_annotations:
            for i in range(7):
#                print("=>",row[i])
#                if re.search(r'\b'+'worker'+r'\b',row[i]):
#                    print("=>",j,row[0],row[1],row[2][:],row[3],row[5],row[6])
                    
                if re.search(r'\b'+'http://dbpedia.org/class/yago/'+r'\b',row[i]):
                    if 'class/yago' in row[i]:
#                    count+=1
                        if j<5:
                            print("=>",j,row[0],row[1],row[2][:],row[3],row[4],row[5],row[6])
                            #caption no
...
#                            print(start_ind)
                            ...
                            yago_type=row[5]
#                        print('\n')
#                        pikes_entities_data_captions.append(pikes_entities_data_C0)
#                        pikes_entities_data_C0=[]
...
#    pikes_entities_data_captions=[]
#    for j in range(1):        
#    caption=image_captions[0]['sentence']
#    entities_annotations_naf=get_entities_annotations_from_text2naf(caption)
    term_saving=[]
    term_noun_saving=[]
    head_id_values=[]
